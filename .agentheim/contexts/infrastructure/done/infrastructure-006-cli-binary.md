...
Two new artefacts plus a documentation update deliver the `kanban` CLI as a second delivery surface alongside the plugin skills, both backed by `lib/lifecycle.js`.

**Files:**
- `package.json` (new) — name `agentheim-kanban-board`, `bin: { kanban: "./bin/kanban.js" }`, `dependencies: {}`, `engines.node: >=18`, MIT.
- `bin/kanban.js` (new, chmod +x, shebang `#!/usr/bin/env node`) — thin dispatcher over `lib/lifecycle.js`. Subcommands: `start | stop | status | open | help | --help | -h | --version | -v`. Resolves `serverPath` relative to `__dirname`; resolves `root` from `process.cwd()`. Exit codes per spec (status: 0 running / 1 not running; stop idempotent; unknown command exits 2).
- `README.md` — added "CLI (alternative to slash commands)" section covering `npm link` install, the four subcommands, and the shared-lock-file note.
- `.agentheim/contexts/infrastructure/README.md` — extended "Key commands" with the CLI binary entry.

**ADRs:**
- `0006-cli-second-delivery-surface.md` — CLI and skills are equal surfaces over `lib/lifecycle.js`; `[[plugin-002]]` will delegate skills to the CLI.
- `0008-packaging-bin-zero-deps.md` — `package.json` with empty deps + `bin` entry; `amends: [0005]` (zero-install preserved because no deps = no install step).
- `0005-plugin-packaging-dependencies.md` — frontmatter updated with `amended_by: [0008]` (bidirectional link).

**Smoke tests (from /tmp, server not running):**
- `node bin/kanban.js --version` → `1.0.0`, exit 0.
- `node bin/kanban.js --help` → usage on stdout, exit 0.
- `node bin/kanban.js status` → "Kanban board is not running", exit 1.
- `node bin/kanban.js bogus` → unknown command on stderr, usage, exit 2.
- `node bin/kanban.js` (no args) → usage on stderr, exit 2.
